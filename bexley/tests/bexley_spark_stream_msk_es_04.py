...
# Infrastructure : Amazon MSK, Amazon Opensearch, Spark cluster on EC2, 
# Author : Addy Clement
...
def main():
    ...
    # set up logging with global scope

    global logger
    logger = logging.getLogger("bexley_logs")
...
def extract_json_from_kafka_stream(bootstrap_server, topic, sasl_username,sasl_password, st_offset,max_offset):
...
def extract_ship_cities_from_mysql_01(host, username, password,query):
    ...
    try:
        ...
        print("Error reading shipping destinations ", ex)
...
def extract_ship_cities_from_mysql(db_url,db_query,db_user,db_password,db_driver='com.mysql.jdbc.Driver') :
    ...
    try:
        ...
        print("Problem loading shipping locations from MySQL RDS Database")
        print("Full Error response : ", e)
        
        custom_log = "Problem loading shipping locations from MySQL RDS Database"
        print(custom_log)
        logger.info(custom_log, extra={"http.request.body.content": custom_log})
        #exit on error
        sys.exit(1)
...
def transform_stream_static_join(kafka_df, mysql_df):
    ...
    try:
        ...
        custom_log = "Problem with stream static join"
        print(custom_log)
        logger.info(custom_log, extra={"http.request.body.content": custom_log})
        #exit on error
        sys.exit(1)
