...
def main():
    ...
    kafka_starting_offset = "earliest"   # "latest"
...
def write_enriched_df_to_console(stream):
    ...
    try:
        # change from append to complete
        stream.writeStream.format("console") \
        .outputMode("append").option("checkpointLocation", "s3a://bexley-checkpointdir/spark/") \
        .option("truncate", False).queryName("bexley-live-stream") \
